 ...
 */
 ...
 import { 
   ...
   Mail, Phone, Plus, Save, AlertCircle, ChevronDown, Pencil, RefreshCw, 
   Clock, Sparkles, Eraser, Cloud, CalendarDays, UserCheck, Download, 
   Upload, LogOut, CalendarPlus, Send, Calculator, Users
 } from 'lucide-react';
 ...
 const colorPalette = [
   // Vermelhos & Rosas
   { name: 'KW Red', hex: '#d50000' }, { name: 'Vivid Red', hex: '#ff1744' }, { name: 'Coral Red', hex: '#ff5252' }, { name: 'Vivid Pink', hex: '#f50057' },
   { name: 'Deep Pink', hex: '#c51162' }, { name: 'Neon Pink', hex: '#ff4081' }, { name: 'Hot Magenta', hex: '#d500f9' }, { name: 'Bright Purple', hex: '#aa00ff' },
   // Roxos & Azuis
   { name: 'Neon Purple', hex: '#7c4dff' }, { name: 'Electric Violet', hex: '#6200ea' }, { name: 'Deep Indigo', hex: '#304ffe' }, { name: 'Royal Blue', hex: '#2962ff' },
   { name: 'Neon Blue', hex: '#448aff' }, { name: 'Vivid Cerulean', hex: '#0091ea' }, { name: 'Electric Cyan', hex: '#00b8d4' }, { name: 'Neon Teal', hex: '#00bfa5' },
   // Verdes
   { name: 'Aqua Green', hex: '#1de9b6' }, { name: 'Neon Green', hex: '#00e676' }, { name: 'Vivid Emerald', hex: '#00c853' }, { name: 'Apple Green', hex: '#43a047' },
   { name: 'Lime Green', hex: '#64dd17' }, { name: 'Acid Green', hex: '#aeea00' }, { name: 'Vivid Yellow', hex: '#fbc02d' }, { name: 'Bright Yellow', hex: '#fdd835' },
   // Amarelos & Laranjas
   { name: 'Golden Yellow', hex: '#f57f17' }, { name: 'Vibrant Amber', hex: '#ffab00' }, { name: 'Light Amber', hex: '#ffca28' }, { name: 'Deep Amber', hex: '#ff9100' },
   { name: 'Neon Orange', hex: '#ff6d00' }, { name: 'Vivid Orange', hex: '#ff3d00' }, { name: 'Deep Orange', hex: '#dd2c00' }, { name: 'Burnt Rust', hex: '#bf360c' }
 ];
 ...
 const App = () => {
   ...
   const [isNewColorDropdownOpen, setIsNewColorDropdownOpen] = useState(false);
   ...
   const [newAgentAbsenceInput, setNewAgentAbsenceInput] = useState('');
   ...
   const checkAvailability = useCallback((agentId, day) => {
     ...
     if (agent.weeklyAbsences?.includes(dow)) return { available: false, reason: `Folga ${daysOfWeek[dow]}` };
     ...
     for (let day = 1; day <= daysInMonth; day++) {
       ...
       for (const shift of shifts) {
         const available = agents.filter(a => {
           if (!checkAvailability(a.id, day).available) return false;
   ...
   // --- Funções de Exportação (Excel, PDF, ICS) ---
     ...
     try {
       ...
       const newAgents = json.map((row, index) => {
         ...
         return {
           ...
           color: colorPalette[Math.floor(Math.random() * colorPalette.length)].hex, weeklyAbsences: [], absences: []
         };
       }).filter(Boolean);
   ...
   const exportAgentsToExcel = () => {
     ...
     try {
       const dataToExport = agents.map(agent => ({
         ...
         'Folgas Fixas': agent.weeklyAbsences?.map(d => daysOfWeek[d]).join(', ') || '',
   ...
   const generateMockAgents = async () => {
     ...
     const mock = firstNames.map((f, i) => ({
       ...
       phone: `91${Math.floor(1000000 + Math.random() * 9000000)}`, color: colorPalette[i % colorPalette.length].hex, weeklyAbsences: [], absences: []
     }));
   ...
   return (
     <div className="min-h-screen bg-slate-50 font-sans text-slate-900 flex flex-col md:flex-row overflow-x-hidden">
       <style>{`.custom-scrollbar::-webkit-scrollbar { width: 5px; height: 5px; } .custom-scrollbar::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 10px; }`}</style>
 
       {/* SIDEBAR */}
       <nav className="hidden md:flex fixed left-0 top-0 h-full w-64 bg-slate-900 text-white z-40 border-r border-slate-800 shadow-2xl flex-col">
               ...
               {isAdmin && (
                 <div className="bg-white p-6 md:p-8 rounded-2xl md:rounded-[32px] shadow-xl border border-slate-200">
                   ...
                   <form onSubmit={async (e) => { e.preventDefault(); if (newAgent.firstName && newAgent.lastName) { const agentToSave = { ...newAgent, id: Date.now().toString() }; const newList = [...agents, agentToSave]; setAgents(newList); await saveAgentsToCloud(newList); setNewAgent({ firstName: '', lastName: '', email: '', phone: '', color: '#d50000', weeklyAbsences: [], absences: [] }); setIsNewColorDropdownOpen(false); } }} className="space-y-6">
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50/50 p-6 md:p-6 rounded-[24px] border-2 border-slate-100">
                        <div className="space-y-4"><div className="flex items-center gap-3 border-b pb-3"><RefreshCw size={16} className="text-red-600" /><h4 className="text-[10px] font-black uppercase text-slate-500">Folgas Semanais Fixas</h4></div><div className="grid grid-cols-7 gap-2">{daysOfWeek.map((day, idx) => { const isAbsent = newAgent.weeklyAbsences.includes(idx); return <button key={day} type="button" onClick={() => { const cur = newAgent.weeklyAbsences; const n = isAbsent ? cur.filter(x => x !== idx) : [...cur, idx]; setNewAgent({...newAgent, weeklyAbsences: n}); }} className={`py-3 rounded-xl text-[10px] font-black uppercase border-2 ${isAbsent ? 'bg-red-600 border-red-600 text-white shadow-md' : 'bg-white border-slate-200 text-slate-400'}`}>{day.substring(0,3)}</button>; })}</div></div>
                        <div className="space-y-4"><div className="flex items-center gap-3 border-b pb-3"><CalendarDays size={16} className="text-red-600" /><h4 className="text-[10px] font-black uppercase text-slate-500">Ausências Pontuais (Férias)</h4></div><div className="flex gap-2"><input type="date" className="flex-1 bg-white border-2 border-slate-100 rounded-xl px-4 py-2 text-xs font-bold focus:border-red-600 outline-none" value={newAgentAbsenceInput} onChange={e => setNewAgentAbsenceInput(e.target.value)} /><button type="button" onClick={() => { if (!newAgentAbsenceInput) return; if (!newAgent.absences.includes(newAgentAbsenceInput)) setNewAgent({...newAgent, absences: [...newAgent.absences, newAgentAbsenceInput].sort()}); setNewAgentAbsenceInput(''); }} className="bg-slate-900 text-white px-4 rounded-xl hover:bg-red-600"><Plus size={16}/></button></div></div>
                     </div>
                     <div className="flex flex-col md:flex-row items-center gap-4">
                       <div className="relative w-full md:w-64">
                          <button type="button" onClick={() => setIsNewColorDropdownOpen(!isNewColorDropdownOpen)} className="w-full bg-slate-50 rounded-xl p-4 flex items-center justify-between border-2 border-transparent hover:border-slate-200"><div className="flex items-center gap-3"><div className="w-6 h-6 rounded-lg shadow-sm" style={{ backgroundColor: newAgent.color }}></div><span className="text-xs font-black uppercase text-slate-500">Cor Identificação</span></div><ChevronDown size={16} /></button>
                          {isNewColorDropdownOpen && <div className="absolute top-full left-0 w-full md:w-[400px] mt-2 bg-white rounded-2xl shadow-2xl border border-slate-100 p-3 z-50 grid grid-cols-8 gap-2 animate-in zoom-in-95">{colorPalette.map(c => <button key={c.hex} type="button" onClick={() => { setNewAgent({...newAgent, color: c.hex}); setIsNewColorDropdownOpen(false); }} className={`aspect-square rounded-xl border-2 transition-all ${newAgent.color === c.hex ? 'border-slate-900 scale-110' : 'border-transparent hover:scale-110'}`} style={{ backgroundColor: c.hex }} />)}</div>}
                       </div>
                       <button type="submit" className="w-full md:flex-1 bg-red-600 text-white p-4 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl active:scale-95 transition-all">Registar Consultor na Cloud</button>
                     </div>
                   </form>
                 </div>
               )}
                   ...
                   <tbody className="divide-y divide-slate-100">
                     {agents.map(agent => (
                       <tr key={agent.id} onClick={() => { setSelectedAgentForFile(agent); setIsEditingProfile(false); }} className="hover:bg-slate-50 cursor-pointer transition-all group">
                         ...
                         <td className="px-6 py-4"><div className="flex flex-wrap gap-1">{daysOfWeek.map((d, idx) => <span key={idx} className={`text-[8px] font-black px-1.5 py-0.5 rounded ${agent.weeklyAbsences?.includes(idx) ? 'bg-red-100 text-red-600 border border-red-200' : 'bg-slate-100 text-slate-300 opacity-40'}`}>{d.substring(0,3)}</span>)}</div></td>
       ...
       {selectedAgentForFile && (
         <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-in fade-in">
           <div className="bg-white w-full max-w-4xl rounded-3xl md:rounded-[40px] shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
             <div className="p-6 md:p-8 text-white relative shrink-0" style={{ backgroundColor: selectedAgentForFile.color }}>
               <div className="flex flex-col md:flex-row items-center gap-6">
                 ...
                 <div className="flex gap-2">
                   ...
                   <button onClick={() => { setSelectedAgentForFile(null); setIsEditingProfile(false); }} className="p-4 bg-black/10 rounded-2xl hover:bg-black/20 text-white"><X size={20} /></button>
                 </div>
               </div>
             </div>
             <div className="p-8 overflow-y-auto custom-scrollbar flex-1 space-y-8"><div className="grid grid-cols-2 gap-8"><div className="space-y-4"><h4 className="text-[10px] font-black uppercase text-slate-400 tracking-[0.3em] border-b pb-2">Contactos</h4><div className="space-y-3"><div className="flex items-center gap-3"><Mail className="text-slate-300" size={18}/> {isEditingProfile ? <input className="bg-slate-50 border-2 rounded-lg px-3 py-1.5 w-full outline-none" value={selectedAgentForFile.email} onChange={e => setSelectedAgentForFile({...selectedAgentForFile, email: e.target.value})} /> : String(selectedAgentForFile.email || '')}</div><div className="flex items-center gap-3"><Phone className="text-slate-300" size={18}/> {isEditingProfile ? <input className="bg-slate-50 border-2 rounded-lg px-3 py-1.5 w-full outline-none" value={selectedAgentForFile.phone} onChange={e => setSelectedAgentForFile({...selectedAgentForFile, phone: e.target.value})} /> : String(selectedAgentForFile.phone || 'Sem Telefone')}</div></div>{isEditingProfile && (<div className="mt-6 relative"><button onClick={() => setIsEditColorDropdownOpen(!isEditColorDropdownOpen)} className="w-full bg-slate-50 rounded-xl p-4 flex justify-between border-2"><div className="flex gap-2"><div className="w-6 h-6 rounded-md" style={{ backgroundColor: selectedAgentForFile.color }}></div><span className="text-[10px] font-black uppercase">Mudar Cor</span></div><ChevronDown size={16}/></button>{isEditColorDropdownOpen && (<div className="absolute top-full left-0 w-full mt-2 bg-white rounded-2xl shadow-2xl p-3 z-50 grid grid-cols-8 gap-2">{colorPalette.map(c => <button key={c.hex} onClick={() => { setSelectedAgentForFile({...selectedAgentForFile, color: c.hex}); setIsEditColorDropdownOpen(false); }} className={`aspect-square rounded-lg border-2 ${selectedAgentForFile.color === c.hex ? 'border-slate-900 scale-110' : 'border-transparent'}`} style={{ backgroundColor: c.hex }} />)}</div>)}</div>)}</div><div className="space-y-4"><h4 className="text-[10px] font-black uppercase text-slate-400 tracking-[0.3em] border-b pb-2">Ausências Pontuais</h4>{isAdmin && (<div className="flex gap-2"><input type="date" className="flex-1 bg-slate-50 border-2 rounded-xl px-3 py-1.5 text-xs font-bold outline-none" value={absenceDateInput} onChange={e => setAbsenceDateInput(e.target.value)} /><button onClick={() => { if (!absenceDateInput) return; const cur = selectedAgentForFile.absences || []; if (!cur.includes(absenceDateInput)) setSelectedAgentForFile({...selectedAgentForFile, absences: [...cur, absenceDateInput].sort()}); setAbsenceDateInput(''); }} className="bg-slate-900 text-white px-4 rounded-xl hover:bg-red-600"><Plus size={16}/></button></div>)}<div className="flex flex-wrap gap-2">{(selectedAgentForFile.absences || []).map(date => (<div key={date} className="bg-slate-100 px-3 py-1.5 rounded-lg flex items-center gap-2"><span className="text-[9px] font-black font-mono">{String(date).split('-').reverse().join('/')}</span>{isAdmin && <button onClick={() => setSelectedAgentForFile({...selectedAgentForFile, absences: selectedAgentForFile.absences.filter(d => d !== date)})} className="text-red-400"><X size={12} /></button>}</div>))}</div></div></div><div className="space-y-4"><div className="flex gap-2 border-b-2 pb-2"><RefreshCw className="text-red-600" size={16} /><h4 className="text-[10px] font-black uppercase text-slate-400">Folgas Fixas</h4></div><div className="grid grid-cols-7 gap-2">{daysOfWeek.map((d, i) => { const is = (selectedAgentForFile.weeklyAbsences || []).includes(i); return <button key={d} onClick={() => { if (!isAdmin) return; const cur = selectedAgentForFile.weeklyAbsences || []; const n = is ? cur.filter(x => x !== i) : [...cur, i]; setSelectedAgentForFile({ ...selectedAgentForFile, weeklyAbsences: n }); }} className={`py-3 rounded-xl text-[10px] font-black uppercase border-2 ${is ? 'bg-red-600 text-white' : 'bg-white text-slate-400'}`}>{d.substring(0,3)}</button>; })}</div></div></div>
         ...
         <div className="fixed inset-0 bg-slate-900/90 flex items-center justify-center z-[100] p-4 animate-in zoom-in-95">
           <div className="bg-white w-full max-w-lg rounded-[48px] overflow-hidden shadow-2xl">
             ...
             <div className="p-8 space-y-2 max-h-[450px] overflow-y-auto custom-scrollbar bg-slate-50/30">
               {agents.map(a => {
                 const av = checkAvailability(a.id, assignmentModal.day);
                 return (
